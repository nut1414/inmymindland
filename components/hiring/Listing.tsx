...
const Listing = ({listingInfo: listingInfo}: {listingInfo: HydratedJobListing}) => {
  ...
  return (
    <div className="flex flex-col gap-2 z-10 place-items-center">
      <div className="text-center box-content h-[14rem] w-48 bg-white text-white pb-8 py-4 rounded-md overflow-hidden transition drop-shadow px-4 relative">
        <div className="w-full">
          <Image width={200} height={120} src={listingInfo.image} alt="rectest" className="rounded-md w-20 h-20"/>
        </div>
        <div className="flex flex-col">
          <div className="flex flex-col w-full">
            <div className=" text-black text-left text-xl truncate">{listingInfo.name}</div>
            <div className=" text-black text-left text-base truncate">{listingInfo.description}</div>
          </div>
          <div className=" flex w-full mt-2 gap-2  overflow-y-hidden overflow-x-scroll">
            {listingInfo.tags?.map((tag: string) => (
              <div key={listingInfo?.uid+i++} className=" bg-black font-light text-xs flex flex-row rounded-full px-3 py-1 place-items-center scroll-smooth ">
